...
async function run() {
  ...
  const userId = "user_2BTdH3yJshxoCcFIUW8PnMp4AJJ";
  
  await fetchSample('PeopleConnections', '/private/users/people-connections', {
    method: 'POST',
    body: JSON.stringify({ people_ids: [personId], user_id: userId }),
    category: 'railway'
  });
  
  await fetchSample('CompanyConnections', '/private/users/company-connections', {
    method: 'POST',
    body: JSON.stringify({ company_ids: [companyId], user_id: userId }),
    category: 'railway'
  });
