...
async function run() {
  const types = ['funding-rounds', 'acquisition', 'ipo'];
  
  for (const type of types) {
    const baseName = type === 'funding-rounds' ? 'Funding' : (type === 'acquisition' ? 'Acquisition' : 'IPO');
    
    await fetchSample(`${baseName}Signals`, `/signals/${type}`, {
      method: 'POST',
      body: JSON.stringify({ page: 0, limit: 5 }),
      category: 'signals'
    });
    
    await fetchSample(`${baseName}Count`, `/signals/${type}/count`, {
      method: 'POST',
      body: JSON.stringify({}),
      category: 'signals'
    });
    
    await fetchSample(`${baseName}Filters`, `/signals/${type}/filters`, {
      category: 'signals'
    });
  }
