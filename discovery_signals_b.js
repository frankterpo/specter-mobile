...
async function run() {
  const types = ['investors', 'revenue', 'strategic'];
  
  for (const type of types) {
    const capitalized = type.charAt(0).toUpperCase() + type.slice(1);
    await fetchSample(`${capitalized}Signals`, `/signals/${type}`, {
      method: 'POST',
      body: JSON.stringify({ page: 0, limit: 5 }),
      category: 'signals'
    });
    
    await fetchSample(`${capitalized}Count`, `/signals/${type}/count`, {
      method: 'POST',
      body: JSON.stringify({}),
      category: 'signals'
    });
    
    await fetchSample(`${capitalized}Filters`, `/signals/${type}/filters`, {
      category: 'signals'
    });
  }
